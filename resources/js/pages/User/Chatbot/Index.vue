...
const sendMessage = async () => {
  ...
  try {
    ...
    console.log('URL:', '/user/chatbot/chat');
    ...
    if (data.success) {
      ...
      errorMessage.value = data.message || 'Error al procesar la respuesta.';
    ...
    errorMessage.value = 'Error de conexión: ' + errorMsg;
    
    messages.value.push({
      role: 'assistant',
      content: '❌ Error de conexión. Verifica la consola del navegador.',
      timestamp: new Date()
    });
  } finally {
...
const clearChat = () => {
  if (confirm('¿Estás seguro de que quieres limpiar la conversación?')) {
    messages.value = [
      {
        role: 'assistant',
        content: '¡Hola! Soy tu asistente técnico de Energy Control. ¿En qué puedo ayudarte?',
        timestamp: new Date()
      }
    ];
  }
...
<template>
  <UserLayout>
    ...
    <div class="h-[calc(100vh-8rem)] flex flex-col">
      <!-- Header -->
      <div class="bg-white rounded-t-lg shadow-lg p-6 flex justify-between items-center">
        <div>
          ...
          <p class="text-gray-600 mt-1">Experto en mantenimiento de sistemas diesel</p>
        </div>
        <button
          @click="clearChat"
          class="bg-gray-200 text-gray-700 px-4 py-2 rounded-lg hover:bg-gray-300 transition text-sm font-semibold flex items-center gap-2"
        >
          ...
          <div class="max-w-4xl mx-auto">
            <div class="flex gap-3">
              <textarea
                ...
                rows="2"
                class="flex-1 px-4 py-3 border border-gray-300 rounded-lg focus:ring-2 focus:ring-orange-500 focus:border-transparent resize-none disabled:bg-gray-100 disabled:cursor-not-allowed"
              ></textarea>
              <button
                @click="sendMessage"
                :disabled="!userInput.trim() || isLoading"
                class="bg-orange-600 text-white px-6 py-3 rounded-lg hover:bg-orange-700 transition disabled:opacity-50 disabled:cursor-not-allowed font-semibold flex items-center gap-2"
              >
